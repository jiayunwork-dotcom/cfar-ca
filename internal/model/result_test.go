...
func TestDetectedIndicesIsolation(t *testing.T) {
	r := buildResult()
	got := r.DetectedIndices()
	if len(got) != 1 || got[0] != 3 {
		t.Fatalf("DetectedIndices() = %v, 期望 [3]", got)
	}
	got[0] = 99
	again := r.DetectedIndices()
	if len(again) != 1 || again[0] != 3 {
		t.Errorf("第二次 DetectedIndices() = %v, 期望仍是 [3]", again)
	}
	if got[0] != 99 {
		t.Error("第一次返回切片被后续 DetectedIndices 写穿")
	}
}
